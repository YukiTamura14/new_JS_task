$(document).ready(function(){
  function score_indicate(){
    ...
    let sum = subject_points.reduce(function(sum, next){
      return sum + next;
    }, 0);
    $('#sum_indicate').text(sum);
    // ここに、上記を参考にして平均点を出力する処理を書き込む
    let avr = Math.round(sum / subject_points.length)
    $('#average_indicate').text(avr);
  };

  function get_achievement(){
    // ここに、ランクの値の文字列（平均点が80点以上なら"A"、60点以上なら"B"、40点以上なら"C"、それ以下なら"D"）を出力する処理を書き込む
      // let avr_mark = document.getElementById('average_indicate')
      let evaluation;
      let avr_mark = $('#average_indicate').text();
      if (avr_mark>=80){
        evaluation = 'A'
      }else if (avr_mark>=60){
        evaluation = 'B'
